...
const securityEmailSchema = (t: I18nT) =>
    z.object({
        code: z.string().min(6, t("auth.securityEmail.placeholder")),
    });

type SecurityEmailFormValues = z.infer<ReturnType<typeof securityEmailSchema>>;
...
export function SecurityEmailForm({ email, flowId: propsFlowId, onSuccess, showProgress = true }: SecurityEmailFormProps) {
    const { t } = useI18n();
    ...
    const resendButtonText = React.useMemo(() => {
        ...
        if (isRunning) return `${t("auth.securityEmail.resend")} ${secondsLeft}s`;
        return t("auth.securityEmail.send");
    ...
    React.useEffect(() => {
        ...
        const initTfa = async () => {
            try {
                const res = await sendVerificationCode(flowId, "Email");
    ...
    } = useForm<SecurityEmailFormValues>({
        resolver: zodResolver(securityEmailSchema(t)),
        criteriaMode: "all",
        defaultValues: {
            code: "",
        },
    ...
    const onSubmit: SubmitHandler<SecurityEmailFormValues> = async (values) => {
        const flowId = propsFlowId || (typeof window !== "undefined" ? sessionStorage.getItem("user-login-flow-id") : null);
        if (!flowId) {
            toast.error(t("auth.verification.missingParams"));
            return;
        ...
        try {
            ...
            if (onSuccess) {
                ...
                const isPhoneRequired = typeof window !== "undefined" ? sessionStorage.getItem("is-phone-required") === "true" : false;
                const is2faRequired = typeof window !== "undefined" ? sessionStorage.getItem("is-2fa-required") === "true" : false;

                if (isPhoneRequired) {
                    router.push(`/${locale}/auth/security/phone`);
    ...
    const onInvalid: SubmitErrorHandler<SecurityEmailFormValues> = (formErrors) => {
    ...
    return (
        <div className="mx-auto flex h-full w-full max-w-[500px] flex-col py-5 lg:h-auto md:p-0">
            <div className="flex flex-col h-full lg:h-auto gap-14">
                {showProgress && <SecurityProgress currentStep="email" progress={50} />}

                <AuthHeading
                    title={t("auth.securityEmail.title")}
                    description={
                        <>
                            <span className="font-bold">{email}</span> {t("auth.securityEmail.description", { email: "" }).trim()}
                        </>
                    }
                />
                ...
                >
                    ...
                    <div>
                        ...
                        >
                            {t("auth.securityEmail.verify")}
                        </Button>
                        <AuthPrompt
                            prompt={t("auth.securityEmail.resendPrompt")}
                            buttonText={t("auth.securityEmail.resend")}
