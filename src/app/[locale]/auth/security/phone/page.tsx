...
export default function SecurityPhonePage() {
  const { t } = useI18n();
  const router = useRouter();
  const params = useParams();
  const locale = (params?.locale as string) || "en";
  const steps = useAuthSteps("success");
  const [phoneNumber, setPhoneNumber] = React.useState("+90 532 *** 6810");
  const [isLoading, setIsLoading] = React.useState(true);

  React.useEffect(() => {
    const flowId = typeof window !== "undefined" ? sessionStorage.getItem("user-login-flow-id") : null;
    const storedPhone = typeof window !== "undefined" ? sessionStorage.getItem("user-phone") : null;
  ...
  return (
    <div className="flex items-center justify-center overflow-hidden w-full">
      <AuthLayout
        heading={t("auth.securityPhone.title")}
        steps={steps}
      ...
      >
        <div className="flex flex-col w-full h-full lg:h-auto items-start">
          ...
          <div className="w-full flex justify-center">
            <SecurityPhoneForm phone={phoneNumber} />
          </div>
        </div>
      </AuthLayout>
    </div>
