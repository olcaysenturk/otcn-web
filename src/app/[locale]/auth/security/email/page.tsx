...
export default function SecurityEmailPage() {
  ...
  const [email, setEmail] = React.useState(COMPANY_CONTACTS.email);
  const [isLoading, setIsLoading] = React.useState(true);

  React.useEffect(() => {
    const flowId = typeof window !== "undefined" ? sessionStorage.getItem("user-login-flow-id") : null;
    const storedEmail = typeof window !== "undefined" ? sessionStorage.getItem("user-email") : null;
  ...
  return (
    <div className="flex items-center justify-center overflow-hidden w-full">
      <AuthLayout
        heading={t("auth.securityEmail.title")}
        steps={steps}
